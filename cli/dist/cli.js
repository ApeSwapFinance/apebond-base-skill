...
import { prepareBatchRedeem, prepareBuy, prepareRedeem, prepareTransfer, prepareZapBuy, } from './prepare.js';
...
function assertTxHash(hash) {
    if (!TX_HASH_RE.test(hash)) {
        throw new Error(`Invalid --hash: expected 0x-prefixed 32-byte hex (64 chars). Got: ${hash}. Use txHash from get_request_status, not requestId.`);
    }
}
function assertBondAddress(addr) {
...
program
    ...
    .action((opts) => {
    printJson(prepareBatchRedeem(opts.bond, opts.ids.split(',').map((s) => s.trim())));
});
program
